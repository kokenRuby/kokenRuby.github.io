...
* このままだとedit画面で他のユーザーが投稿を編集できてしまう. どのようにすれば良いか？
##### hint: current_userという現在のユーザーを取得するメソッドがありましたね

* tweet更新画面で更新されましたと表示されるが、これだとわかりづらい....  更新した情報(タイトル、ツイート、ID)をupdateアクションのviewで表示せよ

* destroyアクションも先ほどのupdateと同様のことが起こっている. これも削除するツイート情報を表示せよ

* tweet->user を取りだし表示する処理を行なったが user->tweet を取り出す処理はまだ行なっていない ある特定のユーザーのツイート一覧を表示する画面を作成せよ

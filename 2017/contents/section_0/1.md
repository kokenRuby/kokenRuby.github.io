...
### そして「centos」の中に配布したファイル(講習中で配布します)を全てドラッグアンドドロップで入れてください。

### 必要なファイルは proxy_settings.sh, Vagrantfile, package.box の三種類です.
...
### たまに[mysql]という文字があるのでその場合はそれより上でかつ[mysqld]より下の部分に書いてください.

### 変更を反映するためにmysqlを再起動しましょう

```
$ sudo systemctl restart mysqld.servie
```
